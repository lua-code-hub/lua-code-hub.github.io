loadstring(game:HttpGet("https://raw.githubusercontent.com/lua-code-hub/lua-code-hub.github.io/refs/heads/main/Lua/Test/loading-Screen.lua",true))()
wait (0.1)
...
local AimSettings = {
    SilentAim = false,
    HitChance = 100,
    TargetPart = "Head", -- Frontlines uses Head hitboxes
    ValidParts = {"Head", "Torso", "UpperTorso", "LowerTorso"} -- Common Frontlines hitboxes
}
...
local function GetClosestPlayerToMouse()
    ...
    -- Get enemies from Frontlines teams
    local enemies = {}
    ...
    for _, player in ipairs(enemies) do
        if player.Character and player.Character:FindFirstChild(AimSettings.TargetPart) then
            ...
            -- Check if player is alive using Frontlines health system
        ...
        if player.Character and player ~= localPlayer then
            ...
            if humanoidRootPart and humanoid and head then
                ...
                if torsoPos.Z > 0 and distance <= Settings.ESP.MaxDistance and passedTeamCheck then
                    -- Box Calculation with team-based colors
                    ...
                    local boxColor = Settings.ESP.Rainbow and Color3.fromHSV(tick() % 5 / 5, 1, 1) or
                                   (player.Team ~= localPlayer.Team and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(0, 255, 0))
                    esp.Box.Color = boxColor
                    esp.Box.Visible = Settings.ESP.Boxes

                    -- Enhanced Health Bar for Frontlines
                    ...
                    -- Precise Head Dot
                    local headCenter = head.CFrame.Position
